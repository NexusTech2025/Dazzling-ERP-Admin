...
const { parseTranscript } = require('./parse_transcript.cjs');
const { extractDiffs } = require('./extract_diff.cjs');
...
if (require.main === module) {
  ...
  if (!target) {
    console.error('Usage: node export_digest.cjs <dir_path_or_id> [--out <output_path>]');
    process.exit(1);
  }
