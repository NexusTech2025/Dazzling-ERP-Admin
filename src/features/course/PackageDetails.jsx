...
import { usePackageDetailQuery, usePackageEnrollmentsQuery, usePackageFeeAccountsQuery } from './hooks/usePackageQueries';
...
const PackageDetails = () => {
  ...
  if (error) return <ErrorState message={error.message} onRetry={() => navigate('/admin/packages')} />;
  if (!pkg) return <ErrorState message="Package not found" onRetry={() => navigate('/admin/packages')} />;
        ...
        <div className="pt-6 lg:pt-10 pb-6 space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              { label: 'Total Revenue', value: totalRevenue ? `₹${totalRevenue.toLocaleString()}` : '—', icon: 'payments', color: 'emerald', sub: pending ? `(₹${pending.toLocaleString()} pending)` : 'No pending dues' }
            ].map((stat, i) => (
