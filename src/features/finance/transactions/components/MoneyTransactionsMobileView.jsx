...
/**
 * Renders a full MobileBaseLayout-wrapped mobile page for the Money Transactions feature.
 * Uses compound slots: Header, StatsSlot, FilterSlot, ListSlot, and ActionBarSlot.
 *
 * @param {Object} props - Component properties.
 * @param {Array<Object>} props.transactions - Filtered and sorted (paginated) transactions list.
 ...
 * @param {Function} props.onOpenForm - Open "Log Transaction" form (null for new).
 * @param {Function} props.onClearSelection - Clear all selected IDs.
 * @param {Function} props.onBulkDelete - Bulk delete handler.
 * @param {Function} props.onManageCategories - Trigger category manager modal.
 * @param {React.ReactNode} props.filterPanel - Pre-rendered TransactionFilterPanel.
 * @param {React.ReactNode} props.kpiGrid - Pre-rendered KpiGrid stats.
 * @param {React.ReactNode} props.pagination - Pre-rendered pagination footer.
 * @returns {React.JSX.Element} MobileBaseLayout-wrapped mobile transactions page.
 */
      ...
      <MobileBaseLayout.ListSlot
        ...
        renderEmptyState={
          <div className="flex flex-col items-center justify-center py-10 border border-dashed border-border-light dark:border-border-dark rounded-xl bg-surface-light dark:bg-surface-dark">
            ...
            <p className="text-sm font-semibold text-text-main dark:text-white">No transactions matching active filters.</p>
          </div>
        }
      >
