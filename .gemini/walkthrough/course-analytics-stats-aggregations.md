...
# Walkthrough - Course Analytics Stats Aggregations & Caches Synchronization

We have successfully implemented course analytics stats aggregation inside the workspace hooks, retrieving pre-cached records synchronously from the TanStack query store.
...
* **`CourseCardV2.jsx`** in [CourseCardV2.jsx](file:///e:/NAST/Dazzling/ERP%20System/dazzling-erp-admin/src/features/course/components/CourseCardV2.jsx):
  ...
  * Injected the 3-column connected metrics statistics block (Batches, Packages, Students) above the footer.
...
### 6. Course Analytics Stats Aggregations
* **`useCourseWorkspaceState.js`** in [useCourseWorkspaceState.js](file:///e:/NAST/Dazzling/ERP%20System/dazzling-erp-admin/src/features/course/hooks/useCourseWorkspaceState.js):
  * Extracted cached Batch, PackageItem, and Student lists synchronously from the Query Client store using `getQueryData`.
  * Computed dynamic batch allocations (`batches_count`), bundle relationships (`packages_count`), and active student enrollment lists (`total_students`) in-memory.
  * Hydrated the filtered courses collection before rendering, yielding zero load flashes.
